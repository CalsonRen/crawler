...
# TODO:
# -Add arguments to specify TLS setting
...
ARGS.add_argument(
        ...
        default=4, help='Limit retries on networks errors'
)
ARGS.add_argument(
        '--max_tasks', action='store', type=int, metavar='N',
        default=100, help='Limit concurrent connection'
)
ARGS.add_argument(
...
ARGS.add_argument(
        '--leninet', action='store_false', dest='strict',
...
ARGS.add_argument(
        '-q', '--quiet', action='store_const', const=0, dest='level', default=2, help='Only log errors'
        )


def fix_url(url):
    """Prefix a schema-less URL with https://."""
    if '://' not in url:
        url = 'http://' + url
    return url
